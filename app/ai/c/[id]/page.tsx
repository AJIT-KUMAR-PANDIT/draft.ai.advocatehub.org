...
interface Message {
  ...
  content: string;
  ts: number;
  hasDoc?: boolean;   // this message generated a doc
}
...
When answering legal questions, be precise, cite relevant IPC/CPC/CrPC sections if applicable.`,
};
...
function MessageBubble({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "flex gap-3 max-w-2xl mx-auto w-full px-4",
    ...
    >
      ...
      <div className={cn(
        "rounded-2xl px-4 py-3 text-sm leading-relaxed max-w-[82%] whitespace-pre-wrap",
        isUser
        ...
        {/* Doc badge — shown when this message produced a document */}
        {msg.hasDoc && onViewDoc && (
          <button
            onClick={onViewDoc}
            className="mt-3 flex items-center gap-2 rounded-xl bg-white border border-violet-200 px-3 py-2 text-xs font-semibold text-violet-700 hover:bg-violet-50 transition-colors shadow-sm"
          >
...
function TypingIndicator() {
  return (
    <motion.div
      ...
      className="flex gap-3 max-w-2xl mx-auto w-full px-4"
    >
...
export default function ChatPage() {
  ...
  // ── Convert AI text → DOCX and open editor ───────────────────
  const openInEditor = useCallback(async (text: string, userPrompt: string) => {
    ...
    try {
      ...
      setDocFile(file);
      setActiveExtension("skills"); // open the panel
    } catch (e) {
      console.error("textToDocx failed", e);
    }
  }, []);
  ...
  const triggerReply = useCallback((baseMessages: Message[], mdl: string) => {
    ...
    // Include legal system prompt
    const apiMessages = [
      LEGAL_SYSTEM_MSG,
      ...baseMessages.map(({ role, content }) => ({ role, content })),
    ];
    // Last user message (for detection)
    ...
    streamChat(
      ...
      async () => {
        ...
        // Auto-detect and convert to DOCX
        const isDoc = isLegalDocument(fullText, lastUserMsg);

        setMessages((prev) => {
          const updated = prev.map((m) =>
            m.id === assistantId ? { ...m, hasDoc: isDoc } : m
          );
          persist(updated, mdl);
          return updated;
        });
        ...
        if (isDoc) {
          await openInEditor(fullText, lastUserMsg);
        }
      },
      // onError
      (err) => {
  ...
  useEffect(() => {
    try {
      ...
      if (!hasReply && savedMsgs.length > 0)
        setTimeout(() => triggerReply(savedMsgs, all[id].model), 50);
    } catch {}
  ...
  return (
    <div className="flex h-screen bg-white overflow-hidden">
      ...
      <div className={cn(
        ...
        hasDocPanel ? "w-[40%] border-r border-zinc-100" : "flex-1"
      )}>
        {/* Top bar */}
        ...
        {/* Messages */}
        <div className="flex-1 overflow-y-auto py-6 space-y-4 pb-[200px]">
          {messages.length === 0 && !streaming && (
            <div className="flex flex-col items-center justify-center h-full gap-3 text-zinc-400 text-sm px-6 text-center">
              ...
              <p>Ask me to draft any legal document — agreements, petitions, contracts, affidavits — and it will open in the editor automatically.</p>
            </div>
          )}
          <AnimatePresence initial={false}>
            {messages.map((msg) => (
              <MessageBubble
                key={msg.id}
                msg={msg}
                onViewDoc={msg.hasDoc ? () => setActiveExtension("skills") : undefined}
              />
            ))}
          </AnimatePresence>
          <AnimatePresence>
        ...
        {/* Fixed input bar */}
        <div
          className="fixed bottom-0 flex justify-center pb-3 pt-10 bg-gradient-to-t from-white via-white/95 to-transparent pointer-events-none"
          style={{ left: 0, right: hasDocPanel ? "60%" : 0 }}
        >
          <div className="pointer-events-auto w-full flex justify-center px-4">
            <AI_Prompt
              ...
              onExtensionSelect={(ext) => {
                ...
                // Clear doc when extension is closed
                if (!ext) { setDocFile(null); setDocText(""); }
              }}
            />
          </div>
        </div>
      ...
      <AnimatePresence>
        {hasDocPanel && (
          ...
          >
            {/* Panel header */}
            <div className="flex items-center gap-2 px-4 py-3 border-b border-zinc-100 shrink-0 bg-white">
              <FileText className="h-4 w-4 text-violet-500" />
            ...
            <div className="flex-1 overflow-hidden">
              <DocPanel
                ...
                onRegenerate={async (instruction) => {
                  // Re-ask AI with the instruction
                  const msg: Message = {
                    id: `user-${uid()}`,
                    role: "user",
                    content: `Please revise the document with this instruction: ${instruction}`,
                    ts: Date.now(),
                  };
